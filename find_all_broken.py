    ...
    with open(job.fn("fix.sh"), "w") as outf:
        ...
        for radius, ff in zip(job.document.radii, job.document.fill_fraction):
            if ff >= MIN_PHI and ff <= MAX_PHI:
                subjob = sproj.open_job({"radius": radius, "dielectric": 16})
                if not subjob.isfile("output.txt"):
                    outf.write(f"echo {subjob}\n")
                    outf.write(
                        f"git add workspace/{job}/workspace/{subjob}/signac_statepoint.json\n"
                    )
                    outf.write(
                        f"python recalculate_band_structure.py -j {job} -s {subjob} --reduce_kp --unique&\n"
                    )
                    outf.write(
                    ...
                    outf.write(
                        f"git add workspace/{job}/workspace/{subjob}/signac_job_document.json\n"
                    )
                    outf.write(
                        f"git add workspace/{job}/workspace/{subjob}/output.txt\n"
                    )
        outf.write(f"rm {job.fn('fix.sh')}\n")
    if len(list(open(job.fn("fix.sh")))) <= 1:
